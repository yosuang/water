...
function main() {
  ...
  if (process.platform === "win32" && prompt.length > MAX_WINDOWS_PROMPT_CHARS) {
    throw new Error(
      `handoff is too large for a reliable Windows command line (${prompt.length} characters)`,
    );
  }

  const promptResult = runHerdr([
    "agent",
    "prompt",
    agentName,
    prompt,
  ...
  if (markerCount < 2) {
    throw new Error(
      `handoff acknowledgment was not observed; marker appeared ${markerCount} time(s)`,
    );
  }
  ...
  process.stdout.write(
    `${JSON.stringify({ agent: agentName, acknowledged: true, marker, status: agentStatus })}\n`,
  );
}
