...
The selected model affects Herdr child agents only. The extension does not change the coordinator model. It dispatches `/skill:herdr-subagent` with prompt-template expansion, leaving this package's `skills/herdr-subagent/SKILL.md` as the workflow source of truth.
