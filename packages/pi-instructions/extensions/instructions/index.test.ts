...
test("loads an instructions directory configured in pi-water.json", async () => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-instructions-config-"));
  const instructionsDir = join(agentDir, "custom-instructions");

  try {
    mkdirSync(instructionsDir);
    writeFileSync(join(instructionsDir, "configured.md"), "# Configured\n\n- Loaded from config.\n");
    writeFileSync(
    ...
    assert.match(result.systemPrompt, /Loaded from config\./u);
    assert.ok(result.systemPrompt.includes(instructionsDir));
  } finally {
